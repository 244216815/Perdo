...
@interface ZJTitleView() {
    ...
    CGFloat _imageWidth;
    BOOL _isShowImage;
    BOOL _isShowDesTitle;
    CGSize _desTitleSize;
}
...
- (void)layoutSubviews {
    ...
    if (!_isShowImage&&!_isShowDesTitle) {
...
- (void)adjustDesTitleSubviewFrame{
    _isShowDesTitle = YES;
    CGRect contentViewFrame = self.bounds;
    contentViewFrame.size.width = [self desTitleViewWidth];
    contentViewFrame.origin.x = (self.frame.size.width - contentViewFrame.size.width)/2;
    self.contentView.frame = contentViewFrame;
    self.label.frame = self.contentView.bounds;
    [self addSubview:self.contentView];
    ...
    contentViewFrame.size.height = _desTitleSize.height + _titleSize.height+3;
    contentViewFrame.origin.y = (self.frame.size.height - contentViewFrame.size.height)*0.5;
    self.contentView.frame = contentViewFrame;
    CGFloat labelHeight = self.contentView.frame.size.height - _desTitleSize.height -3;
    CGRect labelFrame = self.label.frame;
    labelFrame.origin.y = 0;
    labelFrame.size.height = labelHeight;
    self.label.frame = labelFrame;
    self.desLabel.frame = CGRectMake(0, CGRectGetMaxY(labelFrame)+3, self.contentView.width, _desTitleSize.height);
}

- (CGFloat)desTitleViewWidth{
...
- (void)setDesText:(NSString *)desText{
    ...
    self.desLabel.text = desText;
     CGRect bounds = [desText boundingRectWithSize:CGSizeMake(MAXFLOAT, 0.0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.desLabel.font} context:nil];
    _desTitleSize = bounds.size;
}
...
- (void)setSelected:(BOOL)selected {
    ...
    self.desLabel.textColor = selected?[UIColor whiteColor]:[UIColor colorWithHexString:@"#999999"];
}

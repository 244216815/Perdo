...
{
    return [self fastestCompressImageWithSize:size imageSize:0];
}

- (UIImage *)fastestCompressImageWithSize:(CGFloat)size imageSize:(NSUInteger)imageSize
{
    NSData *imageData = [self fastestCompressImageSize:size imageSize:imageSize];
...
{
    return [self fastestCompressImageDataWithSize:size imageSize:0];
}

- (NSData *)fastestCompressImageDataWithSize:(CGFloat)size imageSize:(NSUInteger)imageSize
{
    return [self fastestCompressImageSize:size imageSize:imageSize];
}

#pragma mark - 压缩图片接口
- (NSData *)fastestCompressImageSize:(CGFloat)size imageSize:(NSUInteger)imageSize
{
    @autoreleasepool {
        ...
        /** 设备分辨率 */
        CGSize pixel = [UIImage BEE_appPixel];
        /** 缩放图片尺寸 */
        ...
        imageData = BEE_UIImageJPEGRepresentation(self, 1);
        if (imageLength == 0) {
        ...
        if (size < 100) {
            ...
            compressedImage = [self BEE_scaleWithSize:CGSizeMake(self.size.width * factor, self.size.height * factor)];
        } else if (size < 500) {
            percent = 0.45f;
            /** 缩放图片 */
        ...
        do {
            ...
            if (imageDatalength > targetSize * 2) {
                ...
                if (newSize.width*newSize.height > 200*200) {
                    compressedImage = [self BEE_scaleWithSize:newSize];
                }
            }

            imageData = BEE_UIImageJPEGRepresentation(compressedImage, percent);
            ...
            if (imageDatalength > 0 && imageData.length >= imageDatalength) {
                ...
                compressedImage = [self BEE_scaleWithSize:CGSizeMake(compressedImage.size.width * scale, compressedImage.size.height * scale)];
            }
...
- (NSData *)fastestCompressAnimatedImageDataWithScaleRatio:(CGFloat)ratio
{
    @autoreleasepool {
        ...
        for (UIImage *subImage in self.images) {
            UIImage *compressImage = [subImage BEE_scaleWithSize:imageSize];
            [images addObject:compressImage];
        }
        
        UIImage *aminatedImage = [UIImage animatedImageWithImages:images duration:self.duration];
        
        return BEE_UIImageGIFRepresentation(aminatedImage);
    }
}


#pragma mark - 缩放图片尺寸
- (UIImage*)BEE_scaleWithSize:(CGSize)newSize
{
